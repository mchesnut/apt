...
action :add do
  ...
  Chef::Log.info "DEBUG repo_file starts out as: '" + @repo_file.to_s + "'"

  recipe_eval do
    Chef::Log.info "DEBUG entering recipe_eval block"
    # add key
    ...
    # build repo file
    Chef::Log.info "DEBUG building repository with uri \"#{new_resource.uri}\", distribution \"#{new_resource.distribution}\", components \"#{new_resource.components}\", arch \"#{new_resource.arch}\", deb_src \"#{new_resource.deb_src}\""
    ...
    Chef::Log.info "DEBUG building repo_file with name \"#{new_resource.name}.list\""
  ...
  Chef::Log.info "DEBUG repo_file looks like: '" + @repo_file.to_s + "'"
